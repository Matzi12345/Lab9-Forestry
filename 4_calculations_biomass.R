...
#----------------
Bm_equa <- read.csv("https://raw.githubusercontent.com/Matzi12345/Lab9-Forestry/refs/heads/main/Biomass%20Equation.csv?token=GHSAT0AAAAAADBUOZV3VSUSPQUABOTDTWDGZ7PEHUA") %>%
  dplyr::select(b0, b1, Chojnacky_Code)
...
#----------------
trees <- trees %>%
  dplyr::left_join(Bm_equa, by = "Chojnacky_Code")
#----------------
...
#----------------
sum_u2_bm <- sum_u2_bm %>%
  mutate(bm_pa = biomass * trees$EF[1])
#----------------
